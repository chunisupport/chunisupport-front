...
`src/pages/settings/SettingsPrivacy.tsx`、`SettingsApiToken.tsx`、`SettingsAccountDelete.tsx` は現行ルーティングからは直接使われておらず、`Settings.tsx` と責務が重複している。

必要な対応:

- 今後も個別ページとして維持するか、`Settings.tsx` に統合済みとして削除するかを決める
- 方針確定後に未使用コードを整理して、保守対象を明確化する
...
- 設定機能の実装が 1 系統に整理され、未使用実装が残っていない
...
残る主要課題は、認証手段管理の API 契約確定、設定画面テスト整備、旧設定ページ実装の整理である。
