...
test('通常譜面詳細パスは表示IDと難易度をエンコードする', () => {
  // Given: URL 予約文字と空白を含む表示ID。
  const displayId = 'A/B C'

  // When: 通常譜面詳細パスを生成する。
  const result = buildSongChartDetailPath(displayId, 'MASTER')

  // Then: 表示IDがパス要素として安全にエンコードされ、難易度は小文字クエリになる。
  assert.equal(result, '/songs/A%2FB%20C/chart-detail?diff=master')
})

test("WORLD'S END 譜面詳細パスは表示IDをエンコードする", () => {
  // Given: URL 予約文字と空白を含む表示ID。
  const displayId = 'A/B C'

  // When: WORLD'S END 譜面詳細パスを生成する。
  const result = buildWorldsendChartDetailPath(displayId)

  // Then: 表示IDがパス要素として安全にエンコードされる。
  assert.equal(result, '/songs/worldsend/A%2FB%20C/chart-detail')
})

test('楽曲詳細から譜面詳細へ遷移した state を判定する', () => {
  // Given: 楽曲詳細からの遷移 state。
  const state = CHART_DETAIL_FROM_SONG_DETAIL_STATE

  // When: state を判定する。
  const result = isChartDetailFromSongDetailState(state)

  // Then: 楽曲詳細からの遷移として扱われる。
  assert.equal(result, true)
})

test('異なる state は楽曲詳細からの譜面詳細遷移として扱わない', () => {
