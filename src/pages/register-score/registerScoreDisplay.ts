...
export const courseClassBadgeClass = (courseClass: string): string => {
  ...
  if (normalized === 'extra') {
    return `${COURSE_CLASS_EXTRA_BG_CLASS} text-course-class-text`
  }
  if (normalized === 'inf') {
    return `${COURSE_CLASS_INF_BG_CLASS} text-course-class-text`
  }
  const bgClass = COURSE_CLASS_BG_CLASS[normalized]
  return bgClass ? `${bgClass} text-course-class-text` : 'bg-success-bg text-success'
}
