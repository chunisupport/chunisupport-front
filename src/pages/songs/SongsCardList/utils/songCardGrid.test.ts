...
test('コンテナ幅に入る固定幅カードの列数を求めること', () => {
  // Given: 1列ちょうど・2列に足りない・2列ちょうど・幅0。
  ...
  // When: 列数を求める。
  // Then: カード幅+隙間で割り切れる数だけ並べ、最低1列を保つ。
...
test('カード行数は列数で切り上げること', () => {
  // Given: 5件を2列で並べる。
  const itemCount = 5
  const columnCount = 2

  // When: 行数を求める。
  const rowCount = getSongCardRowCount(itemCount, columnCount)

  // Then: 最後の行が1件でも3行になる。
  assert.equal(rowCount, 3)
  assert.equal(getSongCardRowCount(0, 2), 0)
  assert.equal(getSongCardRowCount(4, 0), 0)
})
...
test('仮想行のスライスは列数分だけ切り出すこと', () => {
  ...
  // When: 2列グリッドの各行を切り出す。
  const firstRow = getSongCardRowSlice(items, 0, 2)
  const lastRow = getSongCardRowSlice(items, 2, 2)

  // Then: 先頭行は2件、最終行は余り1件。
  assert.deepEqual(firstRow, ['a', 'b'])
  assert.deepEqual(lastRow, ['e'])
  assert.deepEqual(getSongCardRowSlice(items, 0, 0), [])
})
