...
import {
  sortSongsByReleaseDescAndIdxDesc,
  useSongsData,
} from "../../stores/songsData";
...
const RANDOM_SONG_LAMP_VALUES = RANDOM_SONG_LAMP_OPTIONS.map(
  (option) => option.value,
);
...
type RandomSongPlayStatus =
  (typeof RANDOM_SONG_PLAY_STATUS_OPTIONS)[number]["value"];

type RandomSongBestFrame =
  (typeof RANDOM_SONG_BEST_FRAME_OPTIONS)[number]["value"];
 ...
 */
const hasInvalidWeightValue = <T extends string>(
  values: Record<T, string>,
): boolean =>
  Object.values(values).some(
    (value) => parseWeightValue(String(value)) === null,
  );
 ...
 */
const fetchMyRandomSongRecordData =
  async (): Promise<MyRandomSongRecordData | null> => {
    try {
...
const formatRandomSongRecordLampLabel = (lamp: RandomSongLampFilter): string =>
  RANDOM_SONG_LAMP_OPTIONS.find((option) => option.value === lamp)?.label ??
  lamp;
  ...
  return (
    <>
      <span class={RESULT_RECORD_SCORE_BADGE_CLASS}>
        ...
        <span class={`ml-1 ${SCORE_RANK_TEXT_CLASS[scoreRank]}`}>
          {scoreRank}
        </span>
      </span>
      ...
      <span class={RESULT_RECORD_OP_BADGE_CLASS}>
...
const RandomSongSelect = <T extends string>(props: {
  ...
  <Select<T>
    ...
    itemComponent={(itemProps) => {
      const label =
        props.options.find((option) => option.value === itemProps.item.rawValue)
...
const RandomSongSelectorPage = (): JSX.Element => {
  ...
  const [minConst, setMinConst] = createSignal(
    RANDOM_SONG_SELECTOR_DEFAULTS.minConst,
  );
  const [maxConst, setMaxConst] = createSignal(
    RANDOM_SONG_SELECTOR_DEFAULTS.maxConst,
  );
  ...
  const [minScore, setMinScore] = createSignal(
    RANDOM_SONG_SELECTOR_DEFAULTS.minScore,
  );
  const [maxScore, setMaxScore] = createSignal(
    RANDOM_SONG_SELECTOR_DEFAULTS.maxScore,
  );
  ...
  const genreOptions = createMemo(() =>
    [...new Set(allCandidates().map((candidate) => candidate.genre))].sort(
  ...
  const bestChartKeys = createMemo(
    () =>
      new Set(
        createRandomSongRecordMap(myRecordData()?.bestRecords ?? []).keys(),
      ),
  );
  const hasMyRecordData = createMemo(
    () => myRecordData() !== null && myRecordData() !== undefined,
  );
  const hasInvalidWeights = createMemo(() =>
    hasInvalidWeightValue(difficultyWeights()),
  );
  const validationMessage = createMemo(() => {
    if (parsedCount() === null)
      return RANDOM_SONG_SELECTOR_COPY.invalidCountMessage;
    ...
    if (
      (minScore().trim() !== "" &&
        (scoreMin === null || Number.isNaN(scoreMin))) ||
      (maxScore().trim() !== "" &&
        (scoreMax === null || Number.isNaN(scoreMax))) ||
    ...
    if (hasInvalidWeights())
      return RANDOM_SONG_SELECTOR_COPY.invalidWeightMessage;

    return null;
  });
  const filteredCandidates = createMemo(() => {
    const basicFilteredCandidates = filterRandomSongCandidates(
      allCandidates(),
      {
  ...
  const handleDraw = (): void => {
    ...
    setResults(
      drawRandomSongs(filteredCandidates(), drawCount, randomSongWeight()),
    );
  };
    ...
    <main class="mx-auto flex w-full max-w-5xl flex-col gap-4 p-4">
      <header class="flex items-start gap-3">
        ...
        <div>
          <h1 class="text-2xl font-semibold">
            {RANDOM_SONG_SELECTOR_COPY.title}
          </h1>
          <p class="mt-1 text-sm text-text-muted">
            {RANDOM_SONG_SELECTOR_COPY.description}
          </p>
        </div>
      </header>
      ...
      <ErrorBoundary fallback={(err) => <LoadError error={err} />}>
        <Show
          ...
          fallback={
            <LoadError error={songsResponse.error ?? versionsResponse.error} />
          }
        >
          <section class="rounded-lg border border-border bg-surface p-4 sm:p-6">
            <Show
              when={!isSongsLoading() && !versionsResponse.loading}
              fallback={<Loading />}
            >
              <form
                class="space-y-5"
                    ...
                    <MultiSelectDropdown
                      ...
                      onToggle={(difficulty) =>
                        setSelectedDifficulties((prev) =>
                          toggleSelectionValue(prev, difficulty),
                        )
                      }
                      onSelectAll={() =>
                    ...
                    <MultiSelectDropdown
                      ...
                      onToggle={(genre) =>
                        setSelectedGenres((prev) =>
                          toggleSelectionValue(prev, genre),
                        )
                      }
                    ...
                    <MultiSelectDropdown
                      ...
                      onToggle={(version) =>
                        setSelectedVersions((prev) =>
                          toggleSelectionValue(prev, version),
                        )
                      }
                            ...
                            <MultiSelectDropdown
                              ...
                              onToggle={(lamp) =>
                                setSelectedLamps((prev) =>
                                  toggleSelectionValue(prev, lamp),
                                )
                              }
                              onSelectAll={() =>
                                setSelectedLamps([...RANDOM_SONG_LAMP_VALUES])
          ...
          <section class="rounded-lg border border-border bg-surface-muted p-4 sm:p-6">
            <div class="mb-3 flex items-center justify-between gap-3">
              <h2 class="text-lg font-semibold">
                {RANDOM_SONG_SELECTOR_COPY.resultLabel}
              </h2>
