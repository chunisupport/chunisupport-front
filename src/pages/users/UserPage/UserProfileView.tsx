...
import type { UserProfileWithRecordsDTO, PlayerDTO, PlayerRecordDTO, HonorDTO  } from "../../../types/api";
...
export const UserProfileView: Component<Props> = (props) => {
  ...
  const playerRating = (sumRating(bestRecords)+sumRating(newRecords)) / (bestRecords.length + newRecords.length);
  ...
  return (
    <div class="m-4">
      ...
      <div class="px-3 py-3 border border-gray-200 shadow-sm rounded-md ">
        ...
        {/* TODO: 称号の背景画像を表示 */}
        <p class="mb-3 p-1 bg-yellow-200 rounded-md text-sm text-center">{honors[0].name}</p>
        <div class="mb-2 flex flex-row items-end justify-between">
          <p class="">Lv. {playerInfo.level}</p>
          <h1 class="flex-1 text-xl font-medium text-center">{playerInfo.name}</h1>
        </div>
        <hr class="mb-2 border-t border-gray-200" />
        <p>Rating {playerRating.toFixed(4)} <span class="text-gray-500">(Best {bestRating.toFixed(4)} / New {newRating.toFixed(4)})</span></p>
