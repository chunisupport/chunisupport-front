  ...
  return (
    ...
    >
      ...
      <div class="relative z-10 flex h-full items-center gap-1">
        ...
        {/* スコア・譜面定数列。AJC幅まで収まるようタイトル側へ省略の逃げ先を残す。 */}
        <div class="flex w-24 shrink-0 flex-col items-end justify-center whitespace-nowrap text-right font-oswald leading-none">
          <span class={`text-xl font-bold ${SCORE_RANK_TEXT_CLASS[scoreRank()]}`}>
            {formatInteger(props.record.score)}
          </span>
          <span class={`flex items-center gap-1 text-xl font-bold ${unknownValueClass()}`}>
            <Show when={props.record.combo_lamp}>
              {(lamp) => (
                <span
                  class={`mt-[2px] ${RECORD_CARD_LAMP_BADGE_CLASS} ${getComboLampBadgeClass(lamp(), props.record.score)}`}
                >
