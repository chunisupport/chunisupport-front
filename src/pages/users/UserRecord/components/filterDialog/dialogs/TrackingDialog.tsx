...
const lampOptionsNotNull = (): LampOptions =>
  LAMP_OPTIONS.filter((lamp): lamp is Exclude<typeof lamp, null> => lamp !== null) as LampOptions
...
const TrackingDialog: Component<TrackingDialogProps> = (props) => {
  ...
  const [trackingScoreMode, setTrackingScoreMode] = createSignal<'number' | 'rank'>('rank')
  ...
  const [trackingLampsRadio, setTrackingLampsRadio] = createSignal<LampOption>('ALL JUSTICE')
    ...
    <Dialog open={props.open} onOpenChange={props.onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content class="fixed z-80 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-lg p-6 w-[90vw] max-w-md border border-gray-300">
          <Dialog.Title class="font-bold mb-2">追跡条件の設定「{targetName()}」</Dialog.Title>
          ...
          <div class="space-y-4">
            <div>
              ...
              >
                ...
                <Checkbox.Label for="tracking-score-enabled">目標スコアを有効化</Checkbox.Label>
              </Checkbox>
                  ...
                  {trackingScoreMode() === 'number' ? (
                    ...
                    >
                      <NumberField.Input
                        ...
                        max={1010000}
                        step={1}
                        class="inline-flex items-center justify-between w-full border rounded px-3 py-2 text-sm bg-white border-gray-300 hover:border-gray-400 focus-visible:outline-2 focus-visible:outline-green-500 focus-visible:outline-offset-2 disabled:bg-gray-100 disabled:text-gray-400"
                        onFocus={(event) => event.currentTarget.select()}
                        onBlur={(event) => setTrackingScoreMin(Number(event.currentTarget.value))}
                        disabled={!trackingScoreEnabled()}
                      />
                    </NumberField>
                  ) : (
                    <Select
                      options={['1点', ...SCORE_RANKS.filter((rank) => rank !== '0点')]}
                    ...
                    >
                      <Select.Trigger class="inline-flex items-center justify-between w-full border rounded px-3 py-2 text-sm bg-white border-gray-300 hover:border-gray-400 focus-visible:outline-2 focus-visible:outline-green-500 focus-visible:outline-offset-2 disabled:bg-gray-100 disabled:text-gray-400">
              ...
              <div class="mt-2">
                <Checkbox
                  ...
                  onChange={(checked) => setTrackingScoreMode(checked ? 'number' : 'rank')}
                  class="flex items-center"
                  disabled={!trackingScoreEnabled()}
                >
            ...
            <div>
              ...
              >
                ...
                <Checkbox.Label for="tracking-lamp-enabled">目標ランプを有効化</Checkbox.Label>
              </Checkbox>
              <RadioGroup
                  ...
                  {(lamp, index) => {
                    ...
                    return (
                      <RadioGroup.Item value={lamp} class="flex items-center">
                        ...
                        <RadioGroup.ItemControl class="h-5 w-5 rounded-full border border-gray-300 bg-gray-50 data-checked:border-green-600 data-checked:bg-green-600 data-checked:text-white flex items-center justify-center mr-2">
          ...
          <div class="text-xs text-gray-500 mt-4">追跡設定はブラウザに保存されます。</div>
