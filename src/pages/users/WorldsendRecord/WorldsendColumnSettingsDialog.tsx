...
const COLUMN_OPTIONS: ColumnOption[] = WORLDSEND_RECORD_COLUMN_DEFINITIONS.map((column) => ({
  id: column.id,
  label: column.label,
}))

const WorldsendColumnSettingsDialog: Component<Props> = (props) => {
  const [selectedColumnIds, setSelectedColumnIds] = createSignal<WorldsendRecordColumnId[]>(
  ...
  const handleChange = (options: ColumnOption[]) => {
    setSelectedColumnIds(
      sortVisibleWorldsendColumnIdsByDefinitionOrder(options.map((option) => option.id))
    )
  }
