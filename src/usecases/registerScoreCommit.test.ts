...
test('normalizePlayerDataResult: 統計のランプ件数がAPI型とずれてnullの場合も空マップへ正規化する', () => {
  // Given: 登録結果の差分は返っているが、統計内の件数マップがnullになったレスポンスを受け取った状態。
  const result = createPlayerDataResult({
    statistics: {
      total_high_score: 1_000_000,
      lamp_counts: {
        clear: null,
        combo: null,
        full_chain: null,
  ...
  // Then: 結果ビューの統計表示が件数マップ参照で例外にならない。
  assert.deepEqual(normalized.statistics.lamp_counts.clear, {})
  assert.deepEqual(normalized.statistics.lamp_counts.combo, {})
  assert.deepEqual(normalized.statistics.lamp_counts.full_chain, {})
})
