...
import { formatNullablePlayerRating, formatPlayerRating, formatRatingFixed2 } from './ratingFormat'
...
test('レーティングは小数点以下2桁で切り捨てられること', () => {
  // Given
  const roundedUpByToFixed = 15.239

  // When
  const result = formatRatingFixed2(roundedUpByToFixed)
...
for (const { value, expected } of recordRatingCases) {
  test(`レーティング ${value} を小数点以下2桁で切り捨て表示すること`, () => {
    // Given / When
    const result = formatRatingFixed2(value)
